...
{
    int i = 0;
    for (i = 0; i < COS16_MAX_HEAP_ALLOCATIONS; i++)
    {
...
{
   return &heap->data[entry->blocks.sindex * COS16_MEMORY_BLOCK_SIZE];
}
...
static struct heap_entry *heap_allocate_blocks(struct heap *heap, int total)
{
    ...
    for (i = 0; i <= COS16_MAX_HEAP_ALLOCATIONS - total; i++)
    {
...
static struct heap_entry* heap_get_entry_for_data_ptr(struct heap* heap, void* ptr)
{
    ...
    if (rel_offset > 0)
    {
        index = rel_offset / COS16_MEMORY_BLOCK_SIZE;
    }
...
struct heap *heap_create(void *ptr)
{
    ...
    #if COS16_FORCE_MEMORY_ALIGNMENT == 1
    if (((unsigned int)ptr % COS16_MEMORY_BLOCK_SIZE) != 0)
    {
        panic("heap_create(): Expecting pointer to be able to divide into COS16_MAX_HEAP_ALLOCATIONS");
    }
    #endif
...
void *heap_malloc(struct heap *heap, int total)
{
    ...
    int total_blocks = total / COS16_MEMORY_BLOCK_SIZE;
    if (total % COS16_MEMORY_BLOCK_SIZE)
    {
        total_blocks += 1;
    }
